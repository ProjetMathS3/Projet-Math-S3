...
public class CadrePrincipal extends JFrame {
    ...
    private static final String[] nomsParametres =
            { "Déplacement proie", "Période reproduction proies", "Taux de reproduction proies", "Mortalité des proies", "Vision proies",
                    "Déplacement prédateur", "Période reproduction prédateurs", "Taux de reproduction prédateurs",
                    "Mortalité des prédateurs", "Vision prédateurs", "Famine", "Tours"};
    private int[] valeurParametres = { 2, 1, 3, 10, 12, 3, 3, 1, 5, 6, 3, 200 }; //Paramètres par défaut
    ...
    private JPanel InitAndAddPanneauDeGauche() {
        ...
        JButton AfficheLodkaVolterra = new JButton("Afficher Lodka Volterra") ;
        AfficheLodkaVolterra.addActionListener(new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {
                //AfficheGraphe GrapheLodka = new AfficheGraphe ("Graphe Lodka-Volterra", app.getProies().size(), valeurParametres[2], valeurParametres[3], valeurParametres[4], app.getPredateurs().size() ,valeurParametres[7], valeurParametres[8], valeurParametres[9]) ;
                AfficheGraphe GrapheLodka = new AfficheGraphe ("Graphe Lodka-Volterra",53000,(float) valeurParametres[1],(float) valeurParametres[2],(float) valeurParametres[3], 9000 ,(float) valeurParametres[6],(float) valeurParametres[7],(float) valeurParametres[8]) ;

                GrapheLodka.pack() ;
                GrapheLodka.setVisible(true);
            }
        });
        panneauDeParametres.add(AfficheLodkaVolterra) ;
