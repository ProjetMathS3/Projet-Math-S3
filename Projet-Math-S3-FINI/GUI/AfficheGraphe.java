...
public class AfficheGraphe extends JFrame
{
    ...
    public AfficheGraphe(final String title, double NbProies, double FreqReprodProies, double TauxReprodProies, double MortaProies,
                         double NbPred, double FreqReprodPred,  double TauxReprodPred, double MortaPred) {
        super( title );
        final XYDataset dataset = CreateEchelleVolterraDataset(NbProies, FreqReprodProies, TauxReprodProies, MortaProies, NbPred, FreqReprodPred, TauxReprodPred, MortaPred) ;
        final JFreeChart chart = createChart( dataset );
        final ChartPanel chartPanel = new ChartPanel( chart );
        chartPanel.setPreferredSize( new java.awt.Dimension( 560 , 370 ) );
        chartPanel.setMouseZoomable( true , false );
    ...
    private XYDataset createVolterraDataset(double NbProie, double NataProie, double MortaProie, double NbPred, double NataPred, double Mortapred)
    {
    	...
    	// Valeurs par d�faut pour avoir un graphe pas trop moche : 53000, 0.09, 0.00001, 9000, 0.000005, 0.25
        final TimeSeriesCollection series = new TimeSeriesCollection();
        final TimeSeries Proies = new TimeSeries ("Proies");
        final TimeSeries Predateurs = new TimeSeries ("Predateurs");
        Month current = new Month();
    ...
    private XYDataset CreateEchelleVolterraDataset ( double NbProies, double FreqReprodProies, double TauxReprodProies, double MortaProies,
                                                     double NbPred, double FreqReprodPred,  double TauxReprodPred, double MortaPred) {
        LodkaVolterra ValeursLodka = new LodkaVolterra(NbProies, FreqReprodProies, TauxReprodProies, MortaProies, NbPred, FreqReprodPred, TauxReprodPred, MortaPred) ;
        // Valeurs par d�faut pour avoir un graphe pas trop moche : 53000, 0.09, 0.00001, 9000, 0.000005, 0.25
        final TimeSeriesCollection series = new TimeSeriesCollection();
        final TimeSeries Proies = new TimeSeries ("Proies");
        final TimeSeries Predateurs = new TimeSeries ("Predateurs");
    ...
    public static void main( final String[ ] args )
    {
        ...
        //final AfficheGraphe GrapheLodka = new AfficheGraphe( title,  53000, 0.05, 0.00001, 9000, 0.000005, 0.25 );
        final AfficheGraphe GrapheLodka = new AfficheGraphe (title, 53000, 1, 3, 10, 9000 ,3, 1, 5) ;
       ...
       //S GrapheProjet.setVisible(true) ;

        /* Ce programme a �t� cod� dans le noir, j'ai un bug sur IntelliJ et je suis dans l'impossibilit� de faire des tests :(
        / La fonction affiche graphe peut s'appeller de deux manni�re :
            - Avec deux ArrayList<int> en parametre elle les affiche simplement leur contenu
            - Avec les valeurs de LodkaVolterra (que l'on est senc� r�cup�rer selon les inputs de l'utilisateur) et fait le calcul auto grace a LodkaVolterra.java
        */
    }
}